...
#### A. 已经开播过（有历史数据）
1. **配置 API** — 右上角「⚙️ API配置」填 Key 并保存
2. **（可选）全局锁定描述** — 首页「🔒 全局锁定提示词描述」填运营硬性要求（极限词禁区等）
3. **创建直播间** — 「🏠 直播间管理」→「新建直播间」→ 选「导入历史场次（已开播）」
4. **导入脚本** — API 自动获取 / 上传 Excel / 手动输入三选一
5. **批量评分** — 点「🎯 完成导入，自动评分选基线」
6. **自动优化** — 「🔧 棘轮分析」→ 选直播间 → 点「🚀 开始自动迭代」
7. **查看结果** — 「🏆 胜出方案」取最高分的提示词和脚本

#### B. 还没开播（冷启动）
1. **配置 API** — 同上
2. **创建直播间** — 「🏠 直播间管理」→「新建直播间」→ 选「🧬 全新直播间（冷启动，未开播）」
3. **贴原始提示词** — 粘贴你准备让数字人跑的完整提示词；保留默认 CTR 3% / 停留 15s 或自定义
4. **一键生成并评分** — 点「🚀 生成脚本并自动评分」→ 系统自动生成首份脚本、建 session、后台打分、设为基线
5. **自动优化** — 直接进「🔧 棘轮分析」开始迭代（与 A 同）
6. **真实数据补位** — 未来有真实直播数据后，到「添加场次」补一场，重评分时系统会把冷启动那场的效果分按百分位重排
...
6. **冷启动** — 没开过播？贴一段原始提示词，系统生成首份脚本 + 用合理默认指标（CTR 3% / 停留 15s）直接进入棘轮迭代
...
**3. 准备数据（或跳过）**
- **有历史数据**：脚本 + 真实 CTR / 停留时长，支持 API 自动获取 / Excel 上传 / 手动输入
- **没有历史数据**：走冷启动模式，只需一段原始提示词即可，系统会自动生成首份脚本并用默认指标合成一场 session
""")
...
**Tab 1「新建直播间」**
输入直播间名称 → 点「✅ 创建直播间」；创建成功后出现「模式」选择器，二选一：

- **导入历史场次（已开播）** — 适合已有真实直播数据的情况，支持 API 自动获取 / 上传 Excel / 手动输入，导入完点「🎯 完成导入，自动评分选基线」，系统后台打分并自动设基线
- **🧬 全新直播间（冷启动，未开播）** — 适合还没开播的情况：
  1. 贴完整原始提示词（数字人/主播要用的那一份）
  2. 设默认 CTR（默认 3%）和停留时长（默认 15s），系统会用这两个值合成一场 session 的实效数据
  3. 选生成模型 + 评分模型
  4. 点「🚀 生成脚本并自动评分」→ 系统串行：保存提示词 → 调生成模型出一份脚本 → 建 session → 后台评分 → 自动设为基线
  5. 完成后直接进「🔧 棘轮分析」开始迭代；未来有真实数据再到「添加场次」补一场，重评分时冷启动那场的实效分会被百分位重排

**Tab 2「添加场次」**
给某个已有直播间增加新场次（可以是真实数据，也可以是冷启动后的补数据）。三种方式：
- API 自动获取（按主播 ID 或按直播间 ID 批量拉）
- 上传 Excel
- 手动输入

导入后脚本 + 指标会保存到该直播间，等待评分。页面会显示该直播间系统内最近一场的日期作为参考。

**Tab 3「管理直播间」**（主战场）
每个直播间展开后能做的事：
- 📌 编辑**原始提示词模板**（导入或冷启动时带的原版，不会被迭代覆盖，保底用）
...
2. 直播间管理 → 新建直播间 → 选「导入历史场次」
3. API 批量导入 6–10 场历史数据 → 点「🎯 完成导入，自动评分选基线」
4. 棘轮分析 → 选基线场次 → 看雷达图找短板
5. 🚀 自动迭代 10 轮 → 等完成
6. 胜出方案里挑最高分的 → 翻译成目标语言 → 下载 txt 给主播/数字人用

**第一次使用（冷启动，未开播）**
1. 首页配 API Key → 写全局锁定描述
2. 直播间管理 → 新建直播间 → 选「🧬 全新直播间（冷启动）」
3. 贴完整原始提示词 → 保留默认 CTR 3% / 停留 15s → 点「🚀 生成脚本并自动评分」
4. 评分完成（约 30 秒–1 分钟）→ 基线自动设好
5. 棘轮分析 → 🚀 自动迭代 → 同已开播流程
6. 首次真实直播结束后回到「添加场次」补一场真实数据，重评分让实效分回到真实基准
...
**Q：冷启动的默认 CTR 3% / 停留 15s 是怎么来的？会不会不准？**
A：这是行业常见的中位偏上值，用作系统起点——单场数据时所有实效指标统一给 5.0 中性分（满分 10），不会偏袒也不会惩罚。真实数据进来后，系统按百分位重排，冷启动那场的实效分会自动修正到应有位置。如果你对你产品的基线有更准的估计，在冷启动表单里直接改两个数字即可。

**Q：冷启动生成的脚本质量靠谱吗？**
A：取决于你贴的原始提示词。建议至少包含：产品名+核心卖点、价格/优惠、目标市场语言、循环结构要求（每 15–30 秒一个闭环）。然后进棘轮分析跑自动迭代，系统会对弱维度逐轮打磨。如果首份脚本质量明显不合格，进「管理直播间」删掉这场 + 重来即可。
""")
