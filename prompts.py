...
## 核心认知（必须贯穿优化）
1. 观众随时进入直播间，平均停留30-45秒
2. 脚本必须由多个30-45秒的"循环"组成，每个循环是一个完整推销闭环
3. 每个循环结构：Hook(3-5秒) → 卖点深度介绍(15-22秒) → 社交证明(3-5秒) → CTA+价格锚(8-13秒)
4. 每个循环只聚焦一个卖点，用数据/场景/原理/对比把它说透；多个卖点分布在不同循环中
5. 从任何位置开始听30-45秒，观众都应获得一个完整的价值主张+购买理由+行动指引
6. **Hook 要强、产品要讲透、CTA 简洁——不要在卖点未说透前重复 CTA，不要高频喊话逼单**
...
## 核心认知（必须贯穿优化）
1. 观众随时进入直播间，平均停留30-45秒
2. 生成的脚本必须由多个30-45秒"循环"组成，每个循环是完整推销闭环
3. 每个循环：Hook(3-5秒) → 卖点深度介绍(15-22秒) → 社交证明(3-5秒) → CTA+价格锚(8-13秒)
4. 每个循环只聚焦一个卖点，用数据/场景/原理/对比把它说透；多个卖点分布在不同循环中
5. **Hook 要强、产品要讲透、CTA 简洁——提示词里要明确禁止高频重复 CTA 和过早 CTA**
